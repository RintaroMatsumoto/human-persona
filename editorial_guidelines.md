...
- **Issue番号・セッションID・ファイルパス等の外部識別子を本文に埋め込む**（記事番号 `#XX` の相互参照は可。#36→#28、#37→#27 など）
...
全記事を `#01〜#38` の連番で運用中（2026-04-06 時点）。ファイル名は `XX-english-slug.md`、frontmatter の title は `"#XX 日本語タイトル"`、emoji は全記事で重複禁止。

- **新規記事**は最後尾（#39 以降）に追加する。過去への挿入・番号繰り上げは原則しない
...
過去記事への相互参照は本文中で `#XX` の形で書いてよい（#36→#28、#37→#27 などが既存実践）。
