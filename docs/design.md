...
**責務**: AI で対応すべきでない状況を検知し、人間への引き継ぎを判定する。

**設計判断**:
- キーワードマッチング + 優先度による多重ルール評価
- 雑談カウンターによる長期化検知
- COMPLAINT/NEGOTIATION 検知時は EmotionStateMachine に
  `problem_detected` を連鎖発火（v0.1 で追加）

**なぜエスカレーションが最優先か**:
`process_message()` では エスカレーション判定 → 感情更新 → 文体選択 の順で処理する。
AI が対応すべきでない状況をまず検知し、それ以降の処理をスキップする設計。
これは安全性の原則に基づく。
...
```json
{
  ...
  "escalation": {
    ...
    "escalation_rules": [
      {
        "reason": "negotiation",
        "keywords": ["単価", "報酬", "値下げ", "割引"],
        "priority": 1
      },
      {
        "reason": "call_request",
        "keywords": ["電話", "通話", "ビデオ", "Zoom", "Teams"],
        "priority": 2
      },
      {
        "reason": "complaint",
        "keywords": ["不満", "クレーム", "おかしい", "問題"],
        "priority": 1
      }
    ]
  }
